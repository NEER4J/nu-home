...
export function PartnerProductForm({ product, categories, isEditing = false, userId }: PartnerProductFormProps) {
  ...
  // Fetch partner's allowed categories
  useEffect(() => {
    async function fetchAllowedCategories() {
      try {
        ...
        const partnerCategories = data
          ?.map(item => item.ServiceCategory as unknown as ServiceCategory)
          .filter(Boolean) || [];
        
        setAllowedCategories(partnerCategories);
        
        // Set initial selected category
        if (partnerCategories.length > 0 && !selectedCategory) {
          setSelectedCategory(partnerCategories[0].service_category_id);
        }
      } catch (error) {
  ...
  useEffect(() => {
    ...
    fetchCategoryFields();
  }, [selectedCategory, isEditing, product, dynamicFieldValues]);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (isEditing && product) {
        ...
        router.push('/partner/products?success=Product updated successfully');
      } else {
        await createProduct(formData);
        router.push('/partner/products?success=Product created successfully');
      }
      
      router.refresh();
    } catch (error) {
      console.error('Error submitting form:', error);
      router.push(`/partner/products?error=${isEditing ? 'Failed to update product' : 'Failed to create product'}`);
      setIsLoading(false);
    }
  };
  ...
  if (allowedCategories.length === 0) {
    return (
      <div className="bg-yellow-50 p-6 rounded-lg border border-yellow-200 text-center">
        ...
        <h3 className="text-lg font-medium text-yellow-800 mb-2">No Approved Categories</h3>
        <p className="text-yellow-700 mb-4">
          You don't have any approved service categories yet. You need at least one approved category to add products.
        </p>
        <div className="mt-4">
          <a
          ...
          <div className="mt-8 pt-6 border-t border-gray-100">
            ...
            {isFetchingFields ? (
              ...
              <div className="bg-gray-50 p-4 rounded-md text-center">
                <p className="text-sm text-gray-500">No custom fields are defined for this category.</p>
              </div>
            )}
          </div>
        )}
