...
export function FieldMapper({ wordpressFields, databaseFields, mapping, onMappingChange }: FieldMapperProps) {
  ...
  const [aiFormatted, setAiFormatted] = useState<any>(null);
  ...
  const handleSuggestMapping = async () => {
    ...
    try {
      const res = await fetch('/api/field-mapping-ai', {
        ...
        body: JSON.stringify({
          wordpressFields: flattenedFields,
          databaseFields,
      ...
      if (data.mapping) {
        onMappingChange(data.mapping);
      } else {
        setAiError('No mapping suggestion received.');
      }
    } catch (err) {
      setAiError('Failed to get mapping suggestion.');
    } finally {
      setAiLoading(false);
    }
  };
  ...
  const handleFormatWithAI = async () => {
    ...
    try {
      ...
      if (data.result && Object.keys(data.result).length > 0) {
        setAiFormatted(data.result);
      } else {
        setAiFormatError('No formatted data received from AI.');
      }
    } catch (err) {
      setAiFormatError('Failed to get formatted data from AI.');
    } finally {
    ...
    <div>
      <div className="flex items-center gap-4 mb-4">
        <button
          ...
          disabled={aiLoading || isLoading || !sampleData}
          className="inline-flex items-center px-4 py-2 border border-blue-600 text-blue-700 bg-white rounded hover:bg-blue-50 disabled:opacity-50"
        >
        ...
        {/* <button
          ...
          disabled={aiFormatLoading || isLoading || !sampleData}
        ...
        >
          {aiFormatLoading ? 'Formatting...' : 'Format WordPress Data with AI'}
        </button> */}
        {aiError && <span className="text-red-600 text-sm">{aiError}</span>}
        {aiFormatError && <span className="text-red-600 text-sm">{aiFormatError}</span>}
      </div>
      {aiFormatted && (
        <div className="mb-6">
          <div className="font-semibold mb-2 text-green-700">AI-Formatted Data Preview</div>
          <pre className="bg-gray-100 p-4 rounded text-xs overflow-x-auto max-h-96">{JSON.stringify(aiFormatted, null, 2)}</pre>
        </div>
      )}
