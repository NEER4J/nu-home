...
export async function POST(request: NextRequest) {
  try {
    ...
    let customerSubject = `Installation Booked - Pay After Completion${companyName ? ' with ' + companyName : ''}`
    let customerHtml = ''
    let customerText = ''
    ...
    if (!customerText) {
      customerText = `Installation Booked - Pay After Completion${companyName ? ' with ' + companyName : ''}\n\n` +
        `Hi ${first_name},\n\n` +
    ...
    let adminSubject = `New Pay After Installation Booking${companyName ? ' - ' + companyName : ''}`
    let adminHtml = ''
    let adminText = ''
    ...
    if (adminEmail) {
      ...
      if (!adminText) {
        adminText = `New Pay After Installation Booking${companyName ? ' - ' + companyName : ''}\n\n` +
          `A customer has booked boiler installation with pay-after-completion option.\n\n` +
          `Customer Information:\n` +
          `Name: ${first_name} ${last_name}\n` +
          `Email: ${email}\n` +
          (phone ? `Phone: ${phone}\n` : '') +
          ...
          `Payment Information:\n` +
          `Payment Method: Pay After Installation\n` +
          `Customer will pay after satisfactory completion of installation.\n\n` +
          `Action Required: Schedule installation and prepare for post-installation payment collection.`
      }
    }
