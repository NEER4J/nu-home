...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      if (notificationSettings.customer.enabled && customerTemplate) {
        try {
      ...
      if (notificationSettings.admin.enabled && adminTemplate && notificationSettings.admin.emails.length > 0) {
