...
export async function POST(request: NextRequest) {
    console.log('🚀🚀🚀 enquiry-submitted-v2 API called (SOLAR) 🚀🚀🚀')
    ...
    try {
        ...
        console.log('enquiry-submitted-v2 (SOLAR) - Parsed hostname:', hostname);
        ...
        const { data: existingMappings } = await supabase
            ...
            .eq('email_type', 'enquiry-submitted')
            .limit(1)
        ...
        // Note: We don't need to save data here - it's already saved by the enquiry form
        ...
        const customerTemplateData = await fieldMappingEngine.mapSubmissionToTemplateFields(finalSubmissionId, 'enquiry-submitted', 'customer')
        ...
        const adminTemplateData = await fieldMappingEngine.mapSubmissionToTemplateFields(finalSubmissionId, 'enquiry-submitted', 'admin')
        ...
        if (mappingsError) {
            ...
            console.log(`📋 Found ${debugMappings?.length || 0} field mappings for enquiry-submitted`)
            if (debugMappings && debugMappings.length > 0) {
                ...
                console.log('⚠️ No field mappings found for enquiry-submitted email type')
            }
        }
        ...
        if (rawSubmissionData) {
            ...
            if (rawSubmissionData.enquiry_data) {
                console.log('📊 Enquiry data structure:', JSON.stringify(rawSubmissionData.enquiry_data, null, 2))
            }
        ...
        // Process templates with field mapping data
        let processedCustomerTemplate = null
        let processedAdminTemplate = null

        if (customerTemplate) {
            console.log('🔧 Processing customer template with field mapping data...')
            processedCustomerTemplate = await fieldMappingEngine.processEmailTemplate(
                {
                    html_template: customerTemplate.html_template,
                    text_template: customerTemplate.text_template,
                    subject: customerTemplate.subject
                },
                customerTemplateData
            )
            console.log('✅ Customer template processed successfully')
        }

        if (adminTemplate) {
            console.log('🔧 Processing admin template with field mapping data...')
            processedAdminTemplate = await fieldMappingEngine.processEmailTemplate(
                {
                    html_template: adminTemplate.html_template,
                    text_template: adminTemplate.text_template,
                    subject: adminTemplate.subject
                },
                adminTemplateData
            )
            console.log('✅ Admin template processed successfully')
        }
        ...
        // Note: GHL lead is created from frontend (in app/solar/enquiry/page.tsx), not backend
        ...
        console.error('❌ enquiry-submitted-v2 error:', error)
        return NextResponse.json({
            error: 'Failed to send enquiry submitted email',
