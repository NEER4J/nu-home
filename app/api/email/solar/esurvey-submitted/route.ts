...
export async function POST(request: NextRequest) {
    console.log('🚀🚀🚀 solar esurvey-submitted API called 🚀🚀🚀')
    ...
    try {
        ...
        console.log('solar esurvey-submitted - Parsed hostname:', hostname);
        ...
        const { data: existingMappings } = await supabase
            ...
            .eq('email_type', 'esurvey-submitted')
            .limit(1)
        ...
        const customerTemplateData = await fieldMappingEngine.mapSubmissionToTemplateFields(finalSubmissionId || '', 'esurvey-submitted', 'customer')
        console.log('📊 Customer template data keys:', Object.keys(customerTemplateData))

        console.log('📊 Mapping admin template data...')
        const adminTemplateData = await fieldMappingEngine.mapSubmissionToTemplateFields(finalSubmissionId || '', 'esurvey-submitted', 'admin')
        ...
        // Note: GHL lead is created from frontend (in UserInfoSection.tsx), not backend
        ...
        return NextResponse.json({
            ...
            message: 'eSurvey emails sent successfully',
            debug: {
        ...
        console.error('❌ solar esurvey-submitted error:', error)
        return NextResponse.json({
            error: 'Failed to send eSurvey email',
