...
export default function HeatingQuotePage({
  ...
  const sendInitialEmailInBackground = async (submissionId: string) => {
    ...
    try {
      ...
      const quoteLink = typeof window !== 'undefined'
        ? `${window.location.origin}/ashp/products?submission=${submissionId}`
        : null
      ...
      if (typeof window !== 'undefined' && navigator.sendBeacon) {
        ...
        const success = navigator.sendBeacon('/api/email/ashp/quote-initial-v2', blob)
        ...
        if (success) {
          ...
          const response = await fetch('/api/email/ashp/quote-initial-v2', {
            method: 'POST',
        ...
        const emailRes = await fetch('/api/email/ashp/quote-initial-v2', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(emailData),
        })
  ...
  const saveInitialFormData = async (contactDetails: any) => {
    ...
    try {
      ...
      if (!isOtpEnabled) {
        router.push(`/ashp/products?submission=${result.data.submission_id}`);
      }
  ...
  const handlePostVerificationCompletion = async (submissionId: string) => {
    ...
    try {
      ...
      const redirectUrl = `/ashp/products?submission=${submissionId}`;
  ...
  return (
    <div className="min-h-[calc(100vh-150px)] relative flex items-center">
      ...
      <IframeNavigationTracker categorySlug="ashp" />
