...
export default function HeatingQuotePage({
  ...
  const sendInitialEmailInBackground = async (submissionId: string) => {
    ...
    try {
      ...
      const quoteLink = typeof window !== 'undefined'
        ? `${window.location.origin}/ev-chargers/products?submission=${submissionId}`
        : null
      ...
      if (typeof window !== 'undefined' && navigator.sendBeacon) {
        ...
        const success = navigator.sendBeacon('/api/email/ev-chargers/quote-initial-v2', blob)
        ...
        if (success) {
          ...
          const response = await fetch('/api/email/ev-chargers/quote-initial-v2', {
            method: 'POST',
        ...
        const emailRes = await fetch('/api/email/ev-chargers/quote-initial-v2', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(emailData),
        })
  ...
  const saveInitialFormData = async (contactDetails: any) => {
    ...
    try {
      ...
      const formData = {
        ...
        status: 'new',
        serviceCategoryName: 'ev-chargers',
      ...
      if (!isOtpEnabled) {
        router.push(`/ev-chargers/products?submission=${result.data.submission_id}`);
      }
  ...
  const handlePostVerificationCompletion = async (submissionId: string) => {
    ...
    try {
      ...
      const redirectUrl = `/ev-chargers/products?submission=${submissionId}`;
