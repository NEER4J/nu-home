...
function SolarAddonsPageContent() {
  ...
  useEffect(() => {
    async function init() {
      try {
        ...
        // 2) Resolve solar service category
        ...
        if (categoryError || !categoryData) {
          setError('Solar category not found')
          return
        }
  ...
  return (
    <div className="max-w-[1500px] mx-auto md:px-6 px-4 py-6 ">
      ...
      {/* Back Link */}
      <div className="mb-6">
        <a href={`/solar/products${submissionId ? `?submission=${submissionId}` : ''}`} className="inline-flex items-center text-gray-600 hover:text-gray-900 transition-colors">
      ...
      <div className="mb-8">
        <h1 className="text-2xl md:text-3xl font-semibold text-gray-900 mb-2">
          Choose add-ons for your {selectedProduct?.name || 'solar system'}
        </h1>
        <p className="text-gray-600">
          Enhance your solar installation with additional components and services
        </p>
      </div>
      ...
      <AddonsLayout
        ...
        backHref={`/solar/products${submissionId ? `?submission=${submissionId}` : ''}`}
...
export default function SolarAddonsPage() {
  return (
    <Suspense fallback={
      <div className="container mx-auto px-4 py-12">
        <AddonsLoader />
